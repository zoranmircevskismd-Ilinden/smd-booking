...
2. Ensure `index.html` exists as the GitHub Pages entry shell
3. Ensure `manifest.json` and `icon.svg` are present if install / home screen behavior matters
...
7. If icon or manifest changed, re-test Add to Home Screen on phone

## Cache note
If the live site appears unchanged, open the URL with a version query, for example:
- `?v=12`

If a phone keeps showing an old home screen icon:
1. Refresh the live URL
2. Close and reopen the browser
3. Delete the old shortcut
4. Add the app to home screen again
...
- Home screen install still works
- Icon and app name appear correctly on phone
